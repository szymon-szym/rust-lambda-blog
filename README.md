...
README below was generated automatically by SAM framework
...
```bash
iot_app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
iot_app$ sam local start-api
iot_app$ curl http://localhost:3000/
```
...
```bash
iot_app$ sam logs -n HelloWorldFunction --stack-name iot_app --tail
```
